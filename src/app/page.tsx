        ...
        <div className="flex justify-between items-end mb-12">
          ...
          <MagneticElement>
            <button 
              ...
              className="flex items-center gap-2 px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full backdrop-blur-md transition-all active:scale-95 cursor-none"
            >
            ...
            <div key={group.title}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {group.assets.map((asset, index) => (
                  <MagneticElement key={asset.symbol}>
                    <motion.div
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: (groupIndex * 0.1) + (index * 0.1) }}
                      className="h-full group relative bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[32px] p-8 overflow-hidden hover:border-white/20 transition-all duration-500 hover:-translate-y-1 cursor-none"
                    >
                      {/* Asset Header */}
